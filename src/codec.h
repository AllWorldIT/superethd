...
extern size_t get_codec_wbuffer_count(uint16_t max_ethernet_frame_size);

extern void reset_packet_encoder(PacketEncoderState *state);
extern int init_packet_encoder(PacketEncoderState *state, uint16_t max_ethernet_frame_size);
extern int packet_encoder(PacketEncoderState *state, BufferList *wbuffers, Buffer *pbuffer, uint16_t mss);

extern int init_packet_decoder(PacketDecoderState *state, uint16_t max_packet_size);
