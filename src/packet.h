...
extern int add_packet_ethernet_header(ethernet_packet_t** pkt, uint16_t* pkt_size, uint8_t dst_mac[ETH_ALEN],
									  uint8_t src_mac[ETH_ALEN], uint16_t ethertype);

extern int add_packet_ipv4_header(ethernet_packet_t** pkt, uint16_t* pkt_size, uint16_t id,
								  uint8_t src_addr[SETH_PACKET_IPV4_IP_LEN], uint8_t dst_addr[SETH_PACKET_IPV4_IP_LEN]);

extern int add_packet_udp_header(ethernet_packet_t** pkt, uint16_t* pkt_size, uint16_t src_port, uint16_t dst_port);
extern int add_packet_icmp_header(ethernet_packet_t** pkt, uint16_t* pkt_size, uint8_t type, uint8_t code);
extern int add_packet_icmp_echo_header(ethernet_packet_t** pkt, uint16_t* pkt_size, uint16_t identifier, uint16_t sequence);
extern int add_payload(ethernet_packet_t** pkt, uint16_t* pkt_size, uint8_t* payload, uint16_t payload_size);
extern int set_length(ethernet_packet_t** pkt, uint16_t pkt_size);
extern int set_checksums(ethernet_packet_t** pkt, uint16_t pkt_size);

extern uint16_t compute_ipv4_header_checksum(ethernet_packet_t* pkt, uint16_t length);
extern uint16_t compute_ipv4_udp_checksum(ethernet_packet_t* pkt, uint16_t length);
extern uint16_t compute_ipv4_icmp_checksum(ethernet_packet_t* pkt, uint16_t length);

extern int is_valid_ethernet(const ethernet_packet_t* packet, uint16_t size);
extern int is_valid_ethernet_ip(const ethernet_packet_t* packet, uint16_t size);
extern int is_valid_ethernet_ipv4(const ethernet_packet_t* packet, uint16_t size);
extern int is_valid_ethernet_ipv4_udp(const ethernet_packet_t* packet, uint16_t size);
extern int is_valid_ethernet_ipv4_icmp(const ethernet_packet_t* packet, uint16_t size);
