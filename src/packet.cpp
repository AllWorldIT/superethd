 ...
 */
void add_packet_ethernet_header(ethernet_packet_t **pkt, uint16_t *pkt_size, uint8_t dst_mac[SETH_PACKET_ETHERNET_MAC_LEN],
...
void add_payload(ethernet_packet_t **pkt, uint16_t *pkt_size, uint8_t *payload, uint16_t payload_size) {
	...
	if (*protocol == SETH_PACKET_IP_PROTOCOL_UDP) {
		...
		// Bump loc with the ICMP4 header length and re-allocate memory
		_realloc_packet(pkt, pkt_size, header_len + payload_size);
		// Copy in payload
		...
		std::ostringstream oss;
		oss << "Unsupported protocol " << std::hex << std::uppercase << *protocol;
		throw PacketNotSupportedEception(oss.str());
	}
}
...
int is_valid_ethernet_ipv4_icmp(const ethernet_packet_t *packet, uint16_t size) {
	...
	if (icmp_packet->type == 8) {
		...
		std::ostringstream oss;
		oss << "Unsupported IPv4 ICMP type " << icmp_packet->type << ", code " << icmp_packet->code;
		throw PacketMalformedEception(oss.str());
	}
