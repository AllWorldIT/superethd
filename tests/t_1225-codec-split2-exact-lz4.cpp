...
TEST_CASE("Check encoding 2 packets, where the second is split between encapsulated packets and fills the second one with LZ4 "
		  "compression",
		  "[codec]") {
	...
	std::shared_ptr<PacketDecoder> decoder = std::make_shared<PacketDecoder>(l2mtu, dec_buffer_pool, avail_buffer_pool);
